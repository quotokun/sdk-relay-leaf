...
└── relay_leaf.dll  # Dynamic library
```
...
### Option 1: Build with GCC (MinGW)

```bash
cd sdk/windows/c/example

# Compile
gcc -Wall -O2 -o example.exe main.c -L../lib -lrelay_leaf
...
### Option 3: Build with MSVC

```bash
cd sdk/windows/c/example
cl /O2 main.c /link /LIBPATH:../lib relay_leaf.lib
copy ..\lib\relay_leaf.dll .
example.exe
```
...
3. Link with the library during compilation

### Auto-Download Native Library
The SDK includes LibraryDownloader (in header). Define RELAY_LEAF_IMPLEMENTATION in ONE .c file:
```c
