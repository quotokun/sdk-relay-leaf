...
namespace RelayLeafSDK
{
    ...
    public static class LibraryDownloader
    {
        ...
        public static string GetLibraryName()
        {
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
                return Environment.Is64BitProcess ? "relay_leaf-windows-x64.dll" : "relay_leaf-windows-x86.dll";
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
                return RuntimeInformation.ProcessArchitecture == Architecture.Arm64
                    ? "librelay_leaf-linux-arm64.so" : "librelay_leaf-linux-x64.so";
    ...
    {
        private const string DllName = "relay_leaf-windows-x64.dll";
