...
func init() {
	...
	dllPath := filepath.Join(dir, "librelay_leaf-linux-x64.so")

	var err error
	dll, err = syscall.LoadDLL(dllPath)
	if err != nil {
		// Try current directory
		dll, err = syscall.LoadDLL("librelay_leaf-linux-x64.so")
		if err != nil {
			panic(fmt.Sprintf("failed to load librelay_leaf-linux-x64.so: %v", err))
		}
	}
