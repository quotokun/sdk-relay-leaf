...
func init() {
	...
	dllPath := filepath.Join(dir, "librelay_leaf-darwin-arm64.dylib")

	var err error
	dll, err = syscall.LoadDLL(dllPath)
	if err != nil {
		// Try current directory
		dll, err = syscall.LoadDLL("librelay_leaf-darwin-arm64.dylib")
		if err != nil {
			panic(fmt.Sprintf("failed to load librelay_leaf-darwin-arm64.dylib: %v", err))
		}
	}
